...
import type { CommitteeScoreSummaryRow } from "./committee-score-store";

export type CommitteeScoreReportVersion = {
  id: string;
  version: number;
...
type ReportVersionStore = {
  versions: CommitteeScoreReportVersion[];
};

const storageDir = process.env.APP_STORAGE_DIR ?? path.join(process.cwd(), "storage");
const storePath = path.join(storageDir, "committee-score-report-versions.json");
let writeQueue: Promise<unknown> = Promise.resolve();

export async function listCommitteeScoreReportVersions() {
  try {
    await ensureDatabaseSchema();
    const [rows] = await db.execute(
      "SELECT id,version_no,source_file_name,created_by_email,created_at,rows_json FROM committee_score_report_versions ORDER BY version_no DESC",
    );
    return (rows as ReportVersionDbRow[]).map(dbRowToVersion).filter(Boolean) as CommitteeScoreReportVersion[];
  } catch (error) {
...
export async function findCommitteeScoreReportVersion(id: string) {
  ...
  try {
    ...
    return await transaction(async (connection) => {
      const [nextRows] = await connection.execute(
        "SELECT COALESCE(MAX(version_no),0)+1 AS next_version FROM committee_score_report_versions FOR UPDATE",
      );
      const version = Math.max(1, Number((nextRows as Array<{ next_version: number | string }>)[0]?.next_version ?? 1));
      const record: CommitteeScoreReportVersion = {
        id: randomUUID(),
        version,
        ...normalized,
      };
      await connection.execute(
        "INSERT INTO committee_score_report_versions (id,version_no,source_file_name,created_by_email,created_at,rows_json) VALUES (?,?,?,?,?,?)",
    ...
    return writeQueued(async () => {
      ...
      const record: CommitteeScoreReportVersion = { id: randomUUID(), version, ...normalized };
...
function normalizeInput(input: { sourceFileName: string; createdByEmail: string; rows: CommitteeScoreSummaryRow[] }) {
  return {
    sourceFileName: input.sourceFileName.trim() || "committee-score-import.xlsx",
    createdByEmail: input.createdByEmail.trim().toLowerCase() || "-",
    createdAt: new Date().toISOString(),
    rows: input.rows.map((row) => ({ ...row, judgeScores: { ...row.judgeScores } })),
  };
}

function dbRowToVersion(row: ReportVersionDbRow): CommitteeScoreReportVersion | null {
  try {
    ...
    return {
      ...
      rows: Array.isArray(parsed) ? parsed as CommitteeScoreSummaryRow[] : [],
    };
  } catch {
    return null;
  }
