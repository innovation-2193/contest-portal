...
export default async function Submit({ searchParams }: { searchParams: Promise<{ registrationCode?: string }> }) {
  ...
  return <><PageHero eyebrow="INNOVATION CONTEST SUBMISSION" title="ลงทะเบียนประกวดนวัตกรรมตำรวจ" description="กรอกข้อมูลผู้สมัคร ข้อมูลผลงานนวัตกรรม และแนบเอกสารประกอบตามแบบฟอร์มให้ครบถ้วนก่อนส่งใบสมัคร"/><section className="wide page-body"><StepRail submission/>{!participantSession && <div className="submission-login-entry"><div><b>เคยส่งผลงานแล้ว?</b><span>เข้าสู่ระบบด้วยอีเมลและ OTP เพื่อดูผลงานของคุณจากอุปกรณ์เครื่องนี้</span></div><Link className="secondary" href="/profile/login"><LogIn/>เข้าสู่ระบบดูผลงาน</Link></div>}<div className="form-layout"><SubmissionForm prefill={activePrefill}/><SideNotes submission/></div></section></>;
}
