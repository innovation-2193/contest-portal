...
import { CalendarDays, CheckCircle2, Download, FileText, Handshake, Lightbulb, Mail, MapPin, Megaphone, Phone, Scale, ShieldCheck, Trophy, Users } from "lucide-react";
import { CriteriaToggle, type CriteriaRound } from "../components/CriteriaToggle";
import { FaqAccordion } from "../components/FaqAccordion";
import { HomeImagePopup } from "../components/HomeImagePopup";
...
const criteriaAward = {
  title: "เงินรางวัลและโล่เกียรติยศ",
  note: "ประกาศผลงานดีเด่นพร้อมโล่เกียรติยศ โดยแบ่งรางวัลหลัก 3 อันดับ และรางวัลชมเชยสำหรับผลงานที่ผ่านเกณฑ์โดดเด่น",
  prizes: [
    { code: "01", title: "รางวัลที่ 1", detail: "ชนะเลิศ" },
    { code: "02", title: "รางวัลที่ 2", detail: "รองชนะเลิศอันดับ 1" },
    { code: "03", title: "รางวัลที่ 3", detail: "รองชนะเลิศอันดับ 2" },
    { code: "+", title: "รางวัลชมเชย", detail: "สำหรับผลงานที่น่าสนใจ" },
  ],
};
const faq=[
  {
    question:"ผู้ที่ไม่ใช่ข้าราชการตำรวจสามารถสมัครเข้าร่วมประกวดได้หรือไม่?",
    answer:["ไม่ได้ โครงการประกวดนวัตกรรมนี้เปิดโอกาสให้เฉพาะข้าราชการตำรวจทุกระดับหรือหน่วยงานตำรวจได้นำเสนอแนวคิดและผลงาน โดยมุ่งเน้นให้เป็นผลงานที่ \"คิดจากตำรวจ ทำโดยตำรวจ\" เพื่อนำมาตอบโจทย์ปัญหาการปฏิบัติงานจริงของเจ้าหน้าที่เท่านั้น"]
  },
  {
  ...
  },
  {
    ...
    answer:["วันที่ 20 - 31 ก.ค. 2569 (ปิดรับเวลา 16.30 น.)"]
  },
  {
...
export default async function Home(){const cookieStore=await cookies();const adminSession=getAdminSession(cookieStore.get(cookieName)?.value);const isAdmin=Boolean(adminSession);const settings=await getAdminSettings();if(!isAdmin&&isPrelanderActive(settings))return <Prelander title={settings.prelanderTitle} message={settings.prelanderMessage} openAt={settings.openAt} adminLoggedIn={isAdmin}/>;const registrationOpen=isEventRegistrationOpen(settings);const submissionOpen=isContestSubmissionOpen(settings);const initialNow=Date.now();const showHomeCountdown=settings.homeCountdownEnabled&&Boolean(settings.homeCountdownTarget);const [winners,news,homePopup]=await Promise.all([listWinners(),listNews({publicOnly:true}),getHomePopup()]);const publishedWinners=winners.filter(w=>w.published);const finalistWinners=publishedWinners.filter(w=>w.rank==="finalist");const awardWinners=publishedWinners.filter(w=>w.rank!=="finalist");return <>
  ...
  <div className="detail-grid contest-detail-grid" id="awards"><article className="criteria-panel contest-detail-card"><div className="criteria-heading contest-summary-heading"><CheckCircle2/><div><span className="eyebrow">หลักการพิจารณา</span><h3>เกณฑ์และรางวัล</h3></div></div><CriteriaToggle rounds={criteriaRounds} award={criteriaAward}/></article></div>
  {publishedWinners.length>0&&<div className="winner-board" id="announcement"><div className="winner-board-head"><div><span className="eyebrow">Announcement</span><h3>ประกาศผลการแข่งขัน</h3><p>แสดงผลแยกตามรอบการแข่งขัน ตั้งแต่รอบคัดเลือกเข้าสู่รอบที่ 2 จนถึงรายชื่อผู้มีสิทธิ์เข้าสู่รอบนำเสนอ</p></div><Trophy/></div><div className="winner-rounds">{finalistWinners.length>0&&<section className="winner-round finalist-round"><div className="winner-round-title"><span>Round 1</span><h4>รายชื่อผลงานที่ผ่านเข้ารอบที่ 2</h4><p>ผู้ผ่านรอบคัดเลือกเตรียมเข้าสู่รอบนำเสนอ/แข่งขันรอบถัดไป</p></div><div className="winner-grid finalist-grid">{finalistWinners.map((winner,index)=><article key={winner.id}><b>{String(index+1).padStart(2,"0")}</b><h4>{winner.projectTitle}</h4><p>{winner.ownerName}</p><small>{winner.division}</small></article>)}</div></section>}{awardWinners.length>0&&<section className="winner-round finalist-announcement-round"><div className="winner-round-title"><span>Finalists</span><h4>10 ทีมสุดท้าย</h4><p>รายชื่อนี้เป็นผู้มีสิทธิ์เข้าสู่การแข่งขันรอบที่ 2 เท่านั้น ไม่มีผลต่อคะแนนในรอบที่สอง</p></div><div className="finalist-announcement-list">{awardWinners.map((winner,index)=><article key={winner.id}><span>{String(index+1).padStart(2,"0")}</span><div><h4>{winner.projectTitle}</h4><p>{winner.ownerName}</p></div></article>)}</div></section>}</div></div>}
  ...
  <div className="faq" id="faq"><span className="eyebrow">คำถามที่พบบ่อย FAQ</span><FaqAccordion items={faq}/></div>
