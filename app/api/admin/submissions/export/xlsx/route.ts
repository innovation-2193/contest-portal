...
export async function GET(request: Request) {
  ...
  const workbook = submissionApplicantsWorkbook(applicants);
...
function submissionApplicantsWorkbook(applicants: SubmissionApplicantExportRow[]) {
  const headers = [
    "รหัสผลงาน",
    "ชื่อผลงาน",
    "ประเภท",
  ...
  const rows = applicants.map((item) => [
    ...
    item.title_th,
    item.submission_type === "team" ? "ทีม" : "เดี่ยว",
    item.team_name ?? "",
    item.member_order === 1 ? "ผู้สมัครหลัก" : `สมาชิก ${item.member_order}`,
    item.title,
    item.first_name,
    item.last_name,
    item.citizen_id,
    ...
    item.email,
    item.phone,
    formatThaiDateTime(item.submitted_at),
  ]);
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets>
    <sheet name="Contest Applicants" sheetId="1" r:id="rId1"/>
  </sheets>
</workbook>`,
    },
    {
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles>
    ...
    },
    {
      ...
      content: worksheetXml([headers, ...rows], headers.length),
    },
    {
      name: "docProps/core.xml",
      content: `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dcmitype="http://purl.org/dc/dcmitype/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <dc:title>Police Innovation Contest 2026 applicants</dc:title>
...
function worksheetXml(rows: string[][], columnCount: number) {
  const columnWidths = [18, 36, 12, 24, 16, 12, 18, 18, 18, 28, 32, 32, 32, 16, 22];
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <sheetData>
${rows.map((row, rowIndex) => `    <row r="${rowIndex + 1}">${row.map((value, columnIndex) => cellXml(value, columnIndex, rowIndex + 1, rowIndex === 0)).join("")}</row>`).join("\n")}
  </sheetData>
</worksheet>`;
}
...
function createXlsx(files: WorkbookFile[]) {
