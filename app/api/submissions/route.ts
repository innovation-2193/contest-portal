...
const fields = z.object({
  ...
  title:z.string().min(1), firstName:z.string().min(2), lastName:z.string().min(2), citizenId:z.string().regex(/^\d{13}$/).refine(isThaiCitizenId,"หมายเลขบัตรประชาชนไม่ถูกต้อง"), phone:z.string().regex(/^\d{9,10}$/), division:z.string().min(2), bureau:z.string().min(2),
...
const memberSchema=z.object({title:z.string().min(1),firstName:z.string().min(2),lastName:z.string().min(2),citizenId:z.string().regex(/^\d{13}$/).refine(isThaiCitizenId,"หมายเลขบัตรประชาชนของสมาชิกไม่ถูกต้อง"),phone:z.string().regex(/^\d{9,10}$/),email:z.string().email(),division:z.string().min(2),bureau:z.string().min(2)});
export async function POST(request:Request){
  try{
    ...
    try{await transaction(async connection=>{
      ...
      await connection.execute("INSERT INTO submission_members(id,submission_id,member_order,title,first_name,last_name,citizen_id,phone,email,division,bureau) VALUES(?,?,?,?,?,?,?,?,?,?,?)",[randomUUID(),submissionId,1,data.title,data.firstName,data.lastName,data.citizenId,data.phone,data.email,data.division,data.bureau]);
      for(const [index,member] of teamMembers.entries())await connection.execute("INSERT INTO submission_members(id,submission_id,member_order,title,first_name,last_name,citizen_id,phone,email,division,bureau) VALUES(?,?,?,?,?,?,?,?,?,?,?)",[randomUUID(),submissionId,index+2,member.title,member.firstName,member.lastName,member.citizenId,member.phone,member.email,member.division,member.bureau]);
    ...
    });}catch(error){await rm(uploadRoot,{recursive:true,force:true});throw error;}
    return NextResponse.json({submissionCode},{status:201});
...
export async function GET(request:Request){const codeValue=new URL(request.url).searchParams.get("code");if(!codeValue)return NextResponse.json({error:"code is required"},{status:400});const [rows]=await db.execute("SELECT s.submission_code,s.submission_type,s.team_name,s.title_th,s.title_en,s.summary,s.video_url,s.status,s.submitted_at,u.email,m.title,m.first_name,m.last_name,m.citizen_id,m.phone,m.division,m.bureau FROM submissions s JOIN users u ON u.id=s.user_id JOIN submission_members m ON m.submission_id=s.id AND m.member_order=1 WHERE s.submission_code=? LIMIT 1",[codeValue]);const row=(rows as unknown[])[0];return row?NextResponse.json(row):NextResponse.json({error:"not found"},{status:404});}
